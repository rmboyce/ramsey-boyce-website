...
def arduino_autoclicker():
    p = Markup('''
        ...
        <p>A normal USB mouse plugs into the USB Host board and the mouse signals are passed through the device. This means that the mouse can be used normally as if it was plugged into the computer without the device (see footnote 1 for details). Then other input devices can be soldered to the Clickotron to generate mouse clicks.</p>
        <p>In my current setup with two foot pedals, the Clickotron has two modes, regular and autoclick. To switch between the modes, hold down both pedals until the autoclick LED turns on or off.</p>
        <p>In regular mode the device will simulate a click for as long as a foot pedal is held down. The left and right click indicator LEDs will also turn on while the Clickotron holds the mouse button down. This allows me to select text by holding the left foot pedal down to hold left click and dragging the mouse across the text.</p>
        ...
        <p>The MAX7221 chip is used to drive the two seven segment displays. The arduino sends SPI signals to the chip through the 2x3 ICSP header to control it. The two capacitors C1 and C2 keep the power stable for the chip. I decided to add the capacitors because many websites say that the MAX7221 is extremely sensitive to power fluctuation, although the capacitors are not strictly necessary.</p>
        
        <h2>Notes</h2>
        <p>Note 1: The current code should work with normal USB mice, but some functions may or may not work depending on the exact mouse you use. If you use a more complicated mouse that reports input differently, you will have to change the code so that the HID report the Clickotron device sends to your computer is correct.</p>
    ''')
...
def cube_timer():
    p = Markup('''
        ...
        <h2>Overview</h2>
        <p>This is a Rubik's Cube timer that I made with Python and PyQt to track how fast the user can solve a cube. The program also saves every past logged time in a text file with timestamps, so you can see your improvement in the long run.</p>
        ...
        <h2>Usage</h2>
        <p>To operate the timer, press and release the spacebar to start (time starts on release) and press the spacebar again to stop. Then you can decide whether to log the time (by pressing space), clear the time, or mark it as a DNF (did not finish: stopped the timer before the cube was completely solved).</p>
        <p>The stats page shows statistics on the previous times in the session (from when the program is opened). The PLL tab  shows some algorithms to permute the last layer of the Rubik's cube.</p>
    ''')
...
def rock_climbing():
    p = Markup('''
        ...
        <p>This is my first outdoor V6! The rock is great quality. If you are tall, try the left heel hook; it's incredible when you set it right.</p>
        ...
        <p>A classic, and my first outdoor V4! The leap for the final hold feels epic to stick.</p>
        <div class="videoContainer">
        ...
        <h2>Serengeti, V5</h2>
        <p>Bishop, CA - Happy Boulders area</p>
        <p>My first V5 outside. There are a lot of ways to do the bottom section, it took a while to figure out the easiest approach for me.</p>
        <div class="videoContainer">
            <iframe width="640" height="360" src="https://www.youtube.com/embed/fzJA3p7K6dU" title="Serengeti rock climb"></iframe>
        </div>
        
        <p>This page is in progess. I'm planning to add the other climbs I've done and my future projects (climbs I'm planning to do).</p>
    ''')
...
def chaos_game():
    ...
    p = Markup('''
        ...
        <p>The Sierpinski Carpet Fraction is the ratio between the size of a polygon and the size of smaller polygons that can be put inside the original against its vertices without colliding the other small polygons. The Sierpinski Fraction causes the points to make nice patterns usually, and is calculated in the method CalculateCarpetFraction() if you want to look more into it.</p>
...
def orbit_sim():
    ...
    p = Markup('''
        ...
        <p>Control by using four clicks: The first two clicks set the orbit, and the last two set the planet radius.</p>
    ''')
